 ...
 */
package com.gofar.cockpits.setup;
...
import com.gofar.cockpits.constants.GofarCockpitsConstants;
 ...
 */
@SystemSetup(extension = GofarCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(GofarCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(GofarCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						GofarCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/gofarcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
